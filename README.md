...
- `offset_commit_on_idle` – Whether to keep committing the stored position on a timer even when no messages are being processed. This keeps idle consumer groups visible to lag monitors (e.g. Burrow) so they don't age out of `__consumer_offsets` retention. Default is `true`.
