...
module Racecar
  class ConsumerSet
    ...
    def initialize(config, logger, instrumenter = NullInstrumenter, threads_orchestrator = nil)
    ...
    def current
      @consumers[@consumer_id_iterator.peek] ||= begin
        ...
        listener = RebalanceListener.new(@config, @instrumenter, @threads_orchestrator)
